...
This lemma is not quite what we need. Since $M \betared[*] M$~for all lambda terms~$M$, $\betared[*]$~is in fact the \emph{reflexive} transitive closure of~$\betared$. The solution to this mismatch is to define a reflexive binary relation on lambda terms similar to~$\betared$ such that many-step~$\beta$-re\-duc\-tion~($\betared[*]$) is this relation's transitive closure. Once we prove that this new relation has the diamond property, we have proven that its transitive closure~$\betared[*]$ has the Church--Rosser property.
