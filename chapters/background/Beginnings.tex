...
\item[$\delta$] is a state transition function, $\delta\from (\Sigma \union B) \times Q \to (\Sigma \union B) \times Q \times D$, describing how the Turing machine reacts to reading a symbol $\sigma$ in state $q$: %CRIT-FIXME: What about halting as an action?
\begin{itemize}
...
\item[\MakeUppercase{$F$}] is the set of \vocab{accepting states}; $F \subset Q$, and if the machine concludes its computation, that is, \vocab{halts} in some state in $F$, this indicates an affirmative answer to the question posed it. %CRIT-FIXME: Clean up def'n of halting using Automata Theory textbook.
\end{description}
